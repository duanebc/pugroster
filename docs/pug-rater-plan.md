# PugRater — M+ Player Rating, Roster & Messaging Addon (Plan Spec)
...
```
In-game events ──► SavedVariables (PugRaterDB.lua)
                        │
              ...
              Python CLI ──► SQLite (canonical store)
                        ...
                        ▼
              PugRater_Lookup.lua (generated data file)
                        │
...
- `PugRaterDB`: runs (with observations + chat), persons, characters, tags, templates, sent-log, settings, refined-lookup cache metadata.
...
- Commands (suggested): `sync` (parse SavedVariables → SQLite, mark exported), `enrich` (Blizzard API: character profile/ilvl/spec validation; Raider.IO API: current + previous season scores), `rate` (refined tier computation), `emit` (write `PugRater_Lookup.lua`), `prune` (archive old-season chat/runs out of the SavedVariables side on next emit), `report` (quick CLI views). `all` runs the pipeline.
...
- `PugRater_Lookup.lua` written into the addon folder, loaded via .toc. Contains refined tiers, RIO scores (this + last season), person-links discovered offline. Addon treats it as read-only enrichment; absence of the file must never break the addon.
...
PugRater/
  ...
  PugRater_Lookup.lua -- generated by companion (checked-in stub)
companion/
  pugrater/           -- python package
...
- All hooks from production code into debug code are guarded feature-detections (`if PugRater.Debug then ...`), so production files load cleanly when the folder is gone.
